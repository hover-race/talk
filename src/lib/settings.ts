...
export function readSettings(): Settings {
  ...
  return {
    head: s.head,
    hairColor: s.hairColor ?? DEFAULT,
    hairLength: s.hairLength,
  };
}

const APPLY: Record<SettingKey, (value: SettingValue) => void> = {
  head: (v) => useTalkStore.getState().setHead(v as HeadId),
  hairColor: (v) =>
    useTalkStore.getState().setHairColor(v === DEFAULT ? null : String(v)),
  hairLength: (v) => useTalkStore.getState().setHairLength(Number(v)),
};
